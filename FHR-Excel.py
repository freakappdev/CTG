...
# Convert array to string list with None as a string
def to_json_compatible(arr):
    return '[' + ', '.join('None' if np.isnan(x) else str(int(x)) for x in arr) + ']'
...
for patient_index, row in df.iterrows():
    ...
    for seg_index, (start, end) in enumerate(segments, 1):
        ...
        output_rows.append({
            ...
            "fhr": to_json_compatible(fhr_seg),
            "ua": to_json_compatible(ua_seg),
            "afm": to_json_compatible(afm_seg)
        })
